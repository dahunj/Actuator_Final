...
void CSetupMoveTab4Dlg::Save_ModelMoveData()
{
	...
	for (int i = 0; i < 6; i++) { strKey.Format("%02d", i);		m_stcNGStageY1[i].GetWindowText(strData);		dData = atof(strData);	INI.Set_Double("38_GOOD_STAGE_Y1", strKey, dData, "%0.3lf"); }
	for (int i = 0; i < 6; i++) { strKey.Format("%02d", i);		m_stcNGStageY2[i].GetWindowText(strData);		dData = atof(strData);	INI.Set_Double("39_GOOD_STAGE_Y2", strKey, dData, "%0.3lf"); }
	for (int i = 0; i < 6; i++) { strKey.Format("%02d", i);		m_stcGoodStageY1[i].GetWindowText(strData);		dData = atof(strData);	INI.Set_Double("40_NG_STAGE_Y1", strKey, dData, "%0.3lf"); }
	for (int i = 0; i < 6; i++) { strKey.Format("%02d", i);		m_stcGoodStageY2[i].GetWindowText(strData);		dData = atof(strData);	INI.Set_Double("41_NG_STAGE_Y2", strKey, dData, "%0.3lf"); }
