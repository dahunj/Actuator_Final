...
void CCommon::Save_MotionPosIO()
{
	...
	gAlm.bStage[ 8] = pDX12->iNGStage1Up;
	gAlm.bStage[ 9] = pDX12->iNGStage2Up;
	gAlm.bStage[10] = pDX11->iGoodStage1Up;
	gAlm.bStage[11] = pDX11->iGoodStage2Up;
...
BOOL CCommon::Check_StageUpDown(CString &sMsg)
{
	...
	if (gAlm.bStage[ 8] != pDX12->iNGStage1Up)			nErrNo = 9;
	if (gAlm.bStage[ 9] != pDX12->iNGStage2Up)			nErrNo = 10;
	if (gAlm.bStage[10] != pDX11->iGoodStage1Up)		nErrNo = 11;
	if (gAlm.bStage[11] != pDX11->iGoodStage2Up)		nErrNo = 12;
...
int CCommon::BarcodeFind(CString sBarcode)
{
	for(int i=1; i<gBar.nCount; i++) {
		if (sBarcode == gBar.sModuleID[i]) {
			return i;
		}
	}
	return 0;
...
BOOL CCommon::BarcodeRead()
{
	CString filePath, fileName, sData;

	gBar.nCount = 0;
	CIniFileCS INI("D:\\Barcode\\BarcodeFileName.ini");
	if (!INI.Check_File()) {
		AfxMessageBox("BarcodeFileName.ini Not Found...");
		return FALSE;
	}

	fileName = INI.Get_String("FILE", "NEME", "");
	if (fileName.Right(4) != ".csv") {
		AfxMessageBox("BarcodeFileName을 수정하세요...");
		return FALSE;
	}
	filePath.Format("D:\\Barcode\\%s", fileName); 
	
	CFile file;
	int i = 0;
    if (file.Open(filePath, CFile::modeRead)) {
        CString line, sBarData;
        while (!(sData = ReadLine(file)).IsEmpty()) { // 한 줄씩 읽기
			if (i >= 4999) break;
			AfxExtractSubString(sBarData, sData, 2, ',');
			gBar.sModuleID[i] = sBarData; i++;
        }
		gBar.nCount = i;
        file.Close();
    } else {
		AfxMessageBox("BarcodeFileName이 없습니다. 파일이름이 맞는지 확인하세요...");
		return FALSE;
	}
	for(int i=1; i<gBar.nCount; i++) {
		gBar.sModuleID[i].Replace("\r", "");
		gBar.sModuleID[i].Replace("\n", "");
	}
	return TRUE;
}

CString CCommon::ReadLine(CFile& file) {
    CString sBuffer, sBuffe, strLine = "";
    char buffer[1024];

    while (file.Read(buffer, 1) == 1) {
		sBuffer.Format("%s", buffer);
		sBuffe = sBuffer.Left(1);
        strLine += sBuffe;
        if (buffer[0] == '\n') {	// 개행 문자 발견
            strLine.TrimRight('\n'); // 개행 문자 제거
            return strLine;
    ...
    if (!strLine.IsEmpty()) { // 파일의 마지막 줄 처리
        return strLine;
    }
    return "";
}
