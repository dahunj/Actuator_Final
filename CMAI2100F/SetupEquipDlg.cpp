...
void CSetupEquipDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CHK_USE_OPP_ALARM,			m_chkUseOppAlarm);
	...
	DDX_Control(pDX, IDC_CHK_USE_BAR_SORT, m_chkUseBarSort);
	DDX_Control(pDX, IDC_LBL_BARSORT,	   m_lblBarSort);
	for (int i = 0; i < 2; i++) DDX_Control(pDX, IDC_RDO_BARSORT_NG + i, m_rdoBarSort[i]);
...
void CSetupEquipDlg::Initial_Controls() 
{
	...
	m_Label[20].Init_Ctrl("¹ÙÅÁ", 11, TRUE, RGB(0xFF, 0xFF, 0xFF), RGB(0x60, 0x60, 0x60));	//LoadPicker Module Check
	...
	m_chkUseOppAlarm.Init_Ctrl("¹ÙÅÁ", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0x40, 0xF0, 0xFF), CCheckCS::emRed, 0);
	...
	m_chkUseBarSort.Init_Ctrl("¹ÙÅÁ", 11, TRUE, RGB(0x00, 0x00, 0x00), RGB(0x90, 0x90, 0xF0), CCheckCS::emRed, 0);
	m_lblBarSort.Init_Ctrl("¹ÙÅÁ", 11, TRUE, RGB(0xFF, 0xFF, 0xFF), RGB(0x60, 0x60, 0x60));
	for (int i = 0; i < 2; i++) m_rdoBarSort[i].Init_Ctrl("¹ÙÅÁ", 11, FALSE, RGB(0x00, 0x00, 0x00), RGB(0xC0, 0xC0, 0xC0), CRadioCS::emRed, 0);
...
void CSetupEquipDlg::Display_EquipData()
{
	...
	if (pEquipData->bUseCosmeticNG) m_rdoTaktLog[1].SetCheck(TRUE);
	...
	m_chkUseOppAlarm.SetCheck(pEquipData->bUseOppAlarm);
	...
	m_chkUseBarSort.SetCheck(gData.bUseBarSort);
	if (gData.nBarSortTray == 1) m_rdoBarSort[0].SetCheck(TRUE);
	else						 m_rdoBarSort[1].SetCheck(TRUE);
...
void CSetupEquipDlg::Save_EquipData()
{
	...
	if (m_chkUseBarSort.GetCheck()) {
		if (m_rdoMESUse[1].GetCheck()) {
			AfxMessageBox("Barcode Sorting ±â´ÉÀº MES¿Í °°ÀÌ »ç¿ëÇÒ¼ö ¾ø½À´Ï´Ù.");
			return;
		}
		if (g_objCommon.BarcodeRead() == FALSE) return;
		if (gBar.nCount < 1) {
			AfxMessageBox("Barcode List¿¡ Data°¡ ¾ø½À´Ï´Ù... ");
			return;
		}
		strData.Format("%d BarcodeÀ» Read ÇÏ¿´½À´Ï´Ù.", gBar.nCount);
	...
	INI.Set_Bool("OPTION", "OPP_ALARM",  m_chkUseOppAlarm.GetCheck());
...
//	if ((pEquipData->bUseMES != m_rdoMESUse[1].GetCheck()) || (pEquipData->bUseMESRcpCheck != m_rdoMESUse2[1].GetCheck()))
	...
	m_stcDelayTime[10].GetWindowText(strData); nData = atoi(strData); INI.Set_Integer("DELAY_TIME", "LOADPICKER_CHECK", nData);
	...
	INI.Set_Bool("BARCODE_SORT", "BAR_SORT",  m_chkUseBarSort.GetCheck());
	if (m_rdoBarSort[0].GetCheck()) INI.Set_Integer("BARCODE_SORT", "SORT_TRAY", 1);	//NG-Tray
	else							INI.Set_Integer("BARCODE_SORT", "SORT_TRAY", 2);	//Good-Tray
...
//	g_objDataManager.Save_MoveData();	//¸ðµ¨À» ¹Ù²å´Ù¸é ¹Ù²ï ¹«ºêµ¥ÀÌÅÍ º¯¼ö¸¦ ÆÄÀÏ¿¡´Ù°¡ ÀúÀåÇØÁà¾ßÇÑ´Ù.
	...
	strData.Format("[Setup Equip] Save Click.  Barcode(%d-%d)", gData.bUseBarSort, gBar.nCount);
	g_objLogFile.Save_HandlerLog(strData);
}
