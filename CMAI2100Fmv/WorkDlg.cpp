...
BOOL CWorkDlg::Work_Start()
{
	...
	if (gData.nMgnTrayCount < 1) {
		g_objCommon.Show_MsgBox(1, "Option화면에서 Marginal 자동완공 Tray 수량을 입력하고 진행하세요.....");
		m_rdoWorkStop.SetCheck(TRUE);
		return FALSE;
	}
...
/*	if (gData.bMemphis) sText.Format("Memphis Model로 Run 진행하시겠습니까?");
	else				sText.Format("Varo Model로 Run 진행하시겠습니까?");
	if (g_objCommon.Show_MsgBox(2, sText) != IDOK){
		m_rdoWorkStop.SetCheck(TRUE);
		return FALSE;
	}
...
/*
	...
	nPickerNo = 2;	nJobNo    = 3;	nModuleNo = 4;
	nPosX     = 1;	nPosY     = 1;
	g_objSequenceMain.Get_TrayPosition(nPickerNo, nJobNo, nPosX, nPosY, dx, dy);
	dx = dx + (60.0 * (nModuleNo - 1));
	sLog.Format("A2 Data [Picker:%d Job:%d Option:%d] PosXY(%d,%d) => (%0.3lf,%0.3lf)", nPickerNo, nJobNo, nModuleNo, nPosX, nPosY, dx, dy);
