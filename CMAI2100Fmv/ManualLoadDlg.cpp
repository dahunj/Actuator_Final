...
void CManualLoadDlg::OnBtnLoadPicker1XClick(UINT nID)
{
	...
	if (nIndex == 5 || nIndex == 6) {
		...
		double dMove;
		if (dCurPosX1 < pMoveData->dLoadPickerX1[3] - 1.0) {
			dMove = pEquipData->dTrayPitchX * nDir;
		} else {
...
//		if (nIndex == 4) { AfxMessageBox(_T("Load Picker1 X축이 Align 위치가 아닙니다.....")); return; }
	...
	if (nIndex == 1 || nIndex == 2 || nIndex == 4) {
		if (!g_objCommon.Get_LoadPickerUp(1) && !g_objCommon.Get_LoadPickerOpen(1, 0)) {
...
void CManualLoadDlg::OnBtnLoadPicker1IOClick(UINT nID)
{
	...
	if (nIndex >= 0 && nIndex < 8) {
		g_objCommon.Set_LoadPickerUp(1, 9, nIndex+1);
	}
	...
	if (nIndex == 32) g_objCommon.Set_LoadPickerUp(1, 9, 0);
	if (nIndex == 33) {
		...
		g_objCommon.Set_LoadPickerDown(1, 9, 0);
	}
	if (nIndex == 34) g_objCommon.Set_LoadPickerOpen(1, 9, 0);
	if (nIndex == 35) g_objCommon.Set_LoadPickerClose(1, 9, 0);
...
void CManualLoadDlg::OnBtnLoadPicker2XClick(UINT nID)
{
	...
	if (nIndex == 5 || nIndex == 6) {
		...
		double dCurPosX2 = g_objAJinAXL.Get_Position(AX_LOAD_PICKER_X2);
		double dMove;
		if (dCurPosX2 < pMoveData->dLoadPickerX2[3] - 1.0) {
			dMove = pEquipData->dTrayPitchX * nDir;
		} else {
			if (gData.bMemphis) dMove = pEquipData->dPitchBtm[1] * nDir;
			else				dMove = pEquipData->dPitchBtm[0] * nDir;
...
//		if (nIndex == 4) { AfxMessageBox(_T("Load Picker2 X축이 Align 위치가 아닙니다.....")); return; }
	...
	if (nIndex == 1 || nIndex == 2 || nIndex == 4) {
		if (!g_objCommon.Get_LoadPickerUp(2) && !g_objCommon.Get_LoadPickerOpen(2, 0)) {
			if (g_objCommon.Show_MsgBox(2, "Picker Close 상태입니다. Down 하시겠습니까?") != IDOK) return;
		}
	}
...
void CManualLoadDlg::OnBtnLoadPicker2IOClick(UINT nID)
{
	...
	if (nIndex == 32) g_objCommon.Set_LoadPickerUp(2, 9, 0);
	if (nIndex == 33) {
		...
		g_objCommon.Set_LoadPickerDown(2, 9, 0);
	}
	if (nIndex == 34) g_objCommon.Set_LoadPickerOpen(2, 9, 0);
	if (nIndex == 35) g_objCommon.Set_LoadPickerClose(2, 9, 0);
...
void CManualLoadDlg::OnBtnLoadStage1IOClick(UINT nID)
{
	...
	double dTray1WorkPosE = pMoveData->dLoadStageY1[2] + dTrayPitch * 9.0 + 0.1;
	double dTray2WorkPosS = pMoveData->dLoadStageY2[2] - 0.1;
	double dTray2WorkPosE = pMoveData->dLoadStageY2[2] + dTrayPitch * 9.0 + 0.1;
