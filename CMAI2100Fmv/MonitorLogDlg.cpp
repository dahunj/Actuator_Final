...
void CMonitorLogDlg::OnBnClickedBtnLogCopy()
{
	...
	strFmPath.Format("%s\\System\\MoveDataMem.ini", gsCurrentDir);
	strToPath.Format("D:\\Log-%s\\System\\MoveDataMem.ini", sLogDate);
	CopyFile(strFmPath, strToPath, FALSE);

	strFmPath.Format("%s\\System\\MoveDataVR.ini", gsCurrentDir);
	strToPath.Format("D:\\Log-%s\\System\\MoveDataVR.ini", sLogDate);
	CopyFile(strFmPath, strToPath, FALSE);
