...
//		double dMove;
//		if (gData.bMemphis) dMove = pEquipData->dPitchBtm[1] * nDir;
//		else				dMove = pEquipData->dPitchBtm[0] * nDir;
...
void CManualUnloadDlg::OnBtnUnloadPicker1IOClick(UINT nID)
{
	...
	if (nIndex >= 0 && nIndex < 8) {
		g_objCommon.Set_UnloadPickerUp(1, 9, nIndex+1);
	}
	...
	if (nIndex >= 8 && nIndex < 16){
		...
		g_objCommon.Set_UnloadPickerDown(1, 9, nIndex-7);
	}

	if (nIndex >= 16 && nIndex < 24) {
		g_objCommon.Set_UnloadPickerOpen(1, 9, nIndex-15);
	}

	if (nIndex >= 24 && nIndex < 32) {
		g_objCommon.Set_UnloadPickerClose(1, 9, nIndex-23);
	}

	if (nIndex == 32) g_objCommon.Set_UnloadPickerUp(1, 9, 0);
	if (nIndex == 33) {
		if (!g_objCommon.Get_UnloadPickerOpen(1, 0) && !g_objCommon.Check_Position(AX_UNLOAD_PICKER_Z1, 0)) {
			if (g_objCommon.Show_MsgBox(2, "Picker Close 상태입니다. Down 하시겠습니까?") != IDOK) return;
		}
		g_objCommon.Set_UnloadPickerDown(1, 9, 0);
	}
	if (nIndex == 34) g_objCommon.Set_UnloadPickerOpen(1, 9, 0);
	if (nIndex == 35) g_objCommon.Set_UnloadPickerClose(1, 9, 0);
...
void CManualUnloadDlg::OnBtnUnloadPicker2IOClick(UINT nID)
{
	...
	if (nIndex >= 0 && nIndex < 8) {
		g_objCommon.Set_UnloadPickerUp(2, 9, nIndex+1);
	}
	...
	if (nIndex >= 8 && nIndex < 16){
		...
		g_objCommon.Set_UnloadPickerDown(2, 9, nIndex-7);
	}

	if (nIndex >= 16 && nIndex < 24) {
		g_objCommon.Set_UnloadPickerOpen(2, 9, nIndex-15);
	}

	if (nIndex >= 24 && nIndex < 32) {
		g_objCommon.Set_UnloadPickerClose(2, 9, nIndex-23);
	}

	if (nIndex == 32) g_objCommon.Set_UnloadPickerUp(2, 9, 0);
	if (nIndex == 33) {
		if (!g_objCommon.Get_UnloadPickerOpen(2, 0) && !g_objCommon.Check_Position(AX_UNLOAD_PICKER_Z2, 0)) {
			if (g_objCommon.Show_MsgBox(2, "Picker Close 상태입니다. Down 하시겠습니까?") != IDOK) return;
		}
		g_objCommon.Set_UnloadPickerDown(2, 9, 0);
	}
	if (nIndex == 34) g_objCommon.Set_UnloadPickerOpen(2, 9, 0);
	if (nIndex == 35) g_objCommon.Set_UnloadPickerClose(2, 9, 0);
...
void CManualUnloadDlg::OnBtnGoodStage1YClick(UINT nID)
{
	...
	if (nIndex == 6 || nIndex == 7) {
		...
		double dTrayPitch = (gData.bMemphis ? pEquipData->dTrayPitchYm : pEquipData->dTrayPitchY);
		double dMove = dTrayPitch * nDir;
...
void CManualUnloadDlg::OnBtnGoodStage1IOClick(UINT nID)
{
	...
	double dTray1WorkPosE = pMoveData->dGoodStageY1[3] + dTrayPitch * 9.0 + 0.1;
	double dTray2WorkPosS = pMoveData->dGoodStageY2[3] - 0.1;
	double dTray2WorkPosE = pMoveData->dGoodStageY2[3] + dTrayPitch * 9.0 + 0.1;
...
void CManualUnloadDlg::OnBtnNGStage1IOClick(UINT nID)
{
	...
	double dTray1WorkPosE = pMoveData->dNGStageY1[3] + dTrayPitch * 9.0 + 0.1;
	double dTray2WorkPosS = pMoveData->dNGStageY2[3] - 0.1;
	double dTray2WorkPosE = pMoveData->dNGStageY2[3] + dTrayPitch * 9.0 + 0.1;
